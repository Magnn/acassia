  ...
  return (
    <div className="flex h-full w-full bg-bg-primary text-primary overflow-hidden">
      ...
      <div className={`flex-shrink-0 border-r border-border bg-bg-sidebar transition-all duration-300 ${viewMode === 'table' ? 'w-0 opacity-0 overflow-hidden' : 'w-[380px]'}`}>
        <div className="p-4 border-b border-border bg-bg-sidebar">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2.5">
              <h2 className="text-base font-bold text-primary">Conversas</h2>
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                title={density === 'comfy' ? 'Visualização compacta' : 'Visualização confortável'}
                className="p-1.5 rounded-lg border border-border text-secondary hover:text-primary hover:bg-bg-primary transition-colors"
              >
                {density === 'comfy' ? <Rows3 className="w-3.5 h-3.5" /> : <StretchHorizontal className="w-3.5 h-3.5" />}
              </button>
              <button 
          ...
          <div className="space-y-2.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-bg-primary border border-border rounded-xl pl-9 pr-4 py-2 text-xs text-primary placeholder:text-secondary outline-none focus:border-indigo-500 transition-all"
              />
            </div>

            {/* Filtros Limpos Unificados (Padrão ChatbotX) */}
            <div className="flex items-center gap-1 overflow-x-auto scrollbar-none pb-0.5">
              {[
                { id: 'all', label: 'Todas' },
                { id: 'ativas', label: 'Robô Ativo' },
                { id: 'pausadas', label: 'Atendente' },
                { id: 'convertidos', label: 'Vendas' },
              ].map((st) => (
                <button
                  key={st.id}
                  onClick={() => setFiltro(st.id)}
                  className={`text-[11px] font-medium px-2.5 py-1 rounded-lg transition-all shrink-0 ${
                    filtro === st.id
                      ? 'bg-zinc-800 text-zinc-100 font-semibold border border-zinc-700 shadow-sm'
                      : 'text-secondary hover:text-primary hover:bg-bg-primary border border-transparent'
                  }`}
                >
                  {st.label}
                </button>
              ...
              <button
                ...
                className={`text-[11px] font-medium px-2 py-1 rounded-lg transition-all shrink-0 flex items-center gap-1 ${
                  scoreFilter === 'hot'
                    ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40'
                    : 'text-secondary hover:text-rose-400 hover:bg-rose-500/10 border border-transparent'
                }`}
                title="Filtrar leads com alta intenção de compra"
              >
