...
def executar_v2(ctx) -> Tuple[List[Acao], str]:
    ...
    if meta.get(R.META_B5_SEQ):
        if (meta.get(R.META_B5_PHASE) or "").strip() == "awaiting_reply":
            if not bool(meta.get("static_mm_b5_entregue")):
                if R.lead_respondeu_texto_ou_midia(ctx) and R.dispatch_em_andamento_recente(meta, "static_mm_b5"):
                    logger.info("event=static_mm_b5_reply_durante_dispatch lead=%s", getattr(ctx, "lead_id", "?"))
